...
class OccupancyGridMap:
    ...
    def mark_visited_idx(self, point_idx):
        ...
        if x_index < 0 or y_index < 0 or y_index >= self.dim_cells[0] or x_index >= self.dim_cells[1]:
    ...
    @staticmethod
    def from_png(filename, cell_size, splash=False):
        """
        ...
        if splash: ogm_data = process(ogm_data) # splash
        # ogm_data = compress(ogm_data) # compress

        ## print out occupancy grid
        # for r in ogm_data[::-1]:
        #     for c in r:
...
def compress(arr, R=6, C=18):
    """
    compresses the image pixel arr according to grid row and col
    """
    rows, cols, res = len(arr), len(arr[0]), []
    R, C = rows // R, cols // C
    for i in range(0, rows, R):
        tmp_row = []
        for j in range(0, cols, C):
            tot = 0
            cnt = 0
            for r in range(R):
                for c in range(C):
                    if i+r < rows and j+c < cols:
                        tot += arr[i+r][j+c]
                        cnt += 1
            tmp_row.append(tot/cnt)
        res.append(tmp_row)

    return res

def splat(arr, row, col):
    """
    splat 8D ard pixel
    """
    SPLAT_AREA = 40
    ...
    for i in range(8):
        for mul in range(SPLAT_AREA):
            y, x = row+mul*r[i], col+mul*c[i]
            if 0 <= y < max_r and 0 <= x < max_c: 
                arr[row+mul*r[i]][col+mul*c[i]] = 1

    return arr

def process(arr):
    """
    splash high thres pixels
    """
    THRESHOLD = 0.8

    for r in range(len(arr)):
        for c in range(len(arr[0])):
            if arr[r][c] != 1 and arr[r][c] > THRESHOLD: splat(arr, r, c)
